Xhgui.callgraph = function(container, data, options) {
    ...
    var nodeStyle = function(node) {
        var ratio = node.value / data.total;
    ...
    var nodeLabel = function (node) {
        if (data.metric == 'ct') {
            ...
            return node.name + ' - ' + data.metric + ' ' + node.metricValue;
        }
    };
    ...
    Xhgui.tooltip(el, {
        ...
        formatter: function (d, i) {
            ...
            if (data.metric == 'ct') {
                ...
                metric = Xhgui.metricName(data.metric);
                label += '<strong>' + metric + ':</strong> ' + ratio + '% ' +
                    ' (' + value + ' <span class="units">' + units + '</span>) ' +
                    '<br />';
            }

            label += '<strong>调用次数:</strong> ' + data.callCount +
                '<br />' +
    ...
    var highlightSubtree = function(root) {
        ...
        for (i = 0, len = edges.length; i < len; i++) {
            svg.select('g.edgePath[data-value=' + edges[i] + '] path').style('stroke-width', 3);
        }
    };
