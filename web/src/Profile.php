...
class Profile
{
    ...
    {
        $this->_data = $profile;
        if (!empty($this->_data['export']) && $convert) {
            if (is_string($this->_data['export'])) {
                $this->_data['export'] = json_decode($this->_data['export'], true);
            }
    ...
    {
        $result = array();
        foreach ($this->_data['export']['profile'] as $name => $values) {
    ...
    public function extractDimension($dimension, $limit)
    {
        ...
        foreach ($slice as $func => $funcData) {
            $extract[] = [
                'key' => ++$i,
                'name' => $func,
                'value' => $this->_metricValue($dimension, $funcData[$dimension])
            ];
        }
        return $extract;
    }
        ...
        foreach ($children as $childName => $metrics) {
            ...
            if (!isset($this->_visited[$childName])) {
                ...
                $this->_nodes[] = array(
                    ...
                    'metricValue' => $this->_metricValue($metric, $metrics[$metric])
                );
            } else {
                $revisit = true;
    ...
    protected function _metricValue($metric, $value)
    {
        if (in_array($metric, $this->_metricTime)) {
            $value = \Util::formatTime($value);
        } else if (in_array($metric, $this->_metricBytes)) {
